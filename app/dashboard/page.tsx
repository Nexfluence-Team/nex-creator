...
/* ─── Icons (SVG, no emojis) ───────────────────────────────────────── */
function EyeIcon() { return <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg> }
function GlobeIcon() { return <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><circle cx="12" cy="12" r="10"/><line x1="2" y1="12" x2="22" y2="12"/><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"/></svg> }
function InboxIcon() { return <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M22 12h-4l-3 3-3-3H2"/><path d="M5.45 5.11 2 12v6a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2v-6l-3.45-6.89A2 2 0 0 0 16.76 4H7.24a2 2 0 0 0-1.79 1.11z"/></svg> }
function VideoIcon() { return <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><rect x="2" y="6" width="16" height="12" rx="2"/><path d="m22 8-4 4 4 4"/></svg> }
function CheckIcon() { return <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="20 6 9 17 4 12"/></svg> }
...
function CopyIcon() { return <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg> }
...
function ExternalIcon() { return <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg> }
function EditIcon() { return <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M17 3l4 4-7 7H10v-4l7-7z"/><path d="M3 21h18"/></svg> }
    ...
    <div style={{ minHeight: '100vh', background: C.bgPage, fontFamily: C.font, color: C.ink }}>
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: isMobile ? '24px 16px' : isTablet ? '28px 20px' : '32px 24px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: cols('repeat(2,1fr)', 'repeat(2,1fr)', 'repeat(4,1fr)'), gap: 12, marginBottom: 24 }}>
          {[
            { label: 'Profile Views',   value: analytics?.views.total ?? 0, sub: `+${analytics?.views.last7 ?? 0} this week`, icon: <EyeIcon /> },
            { label: 'Unique Visitors', value: analytics?.views.unique ?? 0, sub: 'all time', icon: <GlobeIcon /> },
            { label: 'Inbox Messages',  value: (analytics?.inbox.messages ?? 0) + (analytics?.inbox.inquiries ?? 0), sub: `${analytics?.inbox.unread ?? 0} unread`, icon: <InboxIcon /> },
            { label: 'Videos',          value: analytics?.content.videos ?? 0, sub: 'on your portfolio', icon: <VideoIcon /> },
          ].map(s => (
            <div key={s.label} style={{ background: C.bg, borderRadius: C.rLg, padding: '18px 20px', border: C.border, boxShadow: C.shadowCard }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
                <span style={{ color: C.primary, width: 20, height: 20 }}>{s.icon}</span>
              ...
              {[
                ...
                <Link key={item.label} href={item.href} style={{ display: 'flex', alignItems: 'center', gap: 10, textDecoration: 'none', opacity: item.done ? 0.55 : 1 }}>
                  <div style={{
                    ...
                    fontSize: 11, color: item.done ? '#00a85a' : C.primary,
                  }}>{item.done ? <CheckIcon style={{ width: 12, height: 12 }} /> : <PlusIcon style={{ width: 12, height: 12 }} />}</div>
        ...
        <div style={{ background: C.bg, borderRadius: C.rLg, padding: '24px', border: C.border, boxShadow: C.shadowCard, marginBottom: 16 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: cols('1fr', 'repeat(2,1fr)', 'repeat(4,1fr)'), gap: 10 }}>
            {[
              { label: 'Edit header & bio',     href: '/profile', icon: <EditIcon /> },
              { label: 'Add a video',           href: '/profile', icon: <VideoIcon /> },
              { label: 'Add a service package', href: '/profile', icon: '💰' /* still emoji, but we can replace with custom SVG if needed */ },
              { label: 'Check your inbox',      href: '/profile', icon: <InboxIcon /> },
            ].map(a => (
              ...
              >
                <span style={{ width: 18, height: 18, color: C.primary }}>{typeof a.icon === 'string' ? a.icon : a.icon}</span>
                <span style={{ fontSize: 13, fontWeight: 600, color: C.ink }}>{a.label}</span>
                <span style={{ marginLeft: 'auto', color: C.inkDim }}><ArrowRightIcon /></span>
              </Link>
            ))}
          </div>
        </div>
