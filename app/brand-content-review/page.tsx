...
/* ════════════════════════════════════════════════════════════════════
   Content Review — app/brand/campaign/[id]/review/[pieceId]/page.tsx
   Full-page content viewer + review panel.
   Left:  Platform-styled content mock-up (Reel / TikTok / YouTube)
          with live stats.
   Right: Creator info · brief compliance checklist · feedback
          composer (sends to messages) · Approve / Request revision /
          Escalate actions · feedback history.
   ════════════════════════════════════════════════════════════════════ */
...
/* ════════════════════════════════════════════════════════════════════
   BRIEF CHECKLIST — items brand ticks off while reviewing
   ════════════════════════════════════════════════════════════════════ */
interface CheckItem {
  id: string
  type: 'do' | 'dont'
...
const BRIEF_CHECKLIST: CheckItem[] = [
  { id: 'c1', type: 'do',    label: 'Shows authentic before/after results',       required: true  },
  { id: 'c2', type: 'do',    label: 'Product featured in real training context',  required: true  },
  { id: 'c3', type: 'do',    label: 'Third-party testing mentioned',              required: false },
  ...
  { id: 'c7', type: 'dont',  label: 'Visuals not overly filtered or staged',      required: false },
  { id: 'c8', type: 'dont',  label: 'Brand identity unaltered',                   required: true  },
]

/* ════════════════════════════════════════════════════════════════════
   MOCK DATA — Piece p5 (Markus Tamm, revision_requested)
   In a real app this is fetched by pieceId from params.
   ════════════════════════════════════════════════════════════════════ */
const PIECE_DATA: ContentPiece = {
  ...
  format: 'TikTok',
  platform: 'TikTok',
  title: 'Electrolyte vs Recovery Stack — Real Talk',
  caption: 'Ok real talk — I ve been testing @kinetics.lv Recovery Stack vs plain electrolytes for 3 weeks and here s what actually happened to my post-run recovery ⚡ Spoiler: one of them is now permanently in my bag.\n\nFull breakdown in the comments 👇\n\n#RecoveryStack #Kinetics #SportNutrition #MarathonPrep #RunnerLife',
  hashtags: ['#RecoveryStack', '#Kinetics', '#SportNutrition', '#MarathonPrep', '#RunnerLife'],
  submittedDate: 'Jun 20, 2026',
  status: 'revision_requested',
  thumbnailColor: '#2563EB',
  gradientEnd: '#1e40af',
  stats: { views: '48.2K', likes: '3.1K', comments: '287', shares: '412', saves: '198', engagement: '11.4%' },
  creator: {
    id: 'cr2', name: 'Markus Tamm', handle: '@markustamm', initials: 'MT', color: '#2563EB',
    platform: 'TikTok', followers: '96K',
  ...
  feedbackHistory: [
    {
      id: 'fb1',
      date: 'Jun 21, 2026 · 2:14 PM',
      type: 'revision_request',
      text: 'Great energy and the hook is strong! One issue — the video implicitly positions our product against a competitor (0:38 mark). Please re-edit to remove the competitor product from frame or replace with a generic "electrolyte drink" reference. Also could you add the affiliate link in bio before resubmitting?',
    },
  ],
}

/* Simulate other pieces for the same creator (navigation) */
const PIECE_P4: Partial<ContentPiece> = {
  id: 'p4', pieceNumber: 1, totalPieces: 2, format: 'TikTok', platform: 'TikTok',
  title: 'Race Day Morning Routine ft. Recovery Stack', status: 'approved',
  thumbnailColor: '#2563EB', gradientEnd: '#1d4ed8',
}
...
function HeartIcon({ s = 15 }: { s?: number }) {
...
function ShareIcon({ s = 15 }: { s?: number }) {
  return <svg width={s} height={s} viewBox="0 0 24 24" fill="none"><path d="M4 12v8a2 2 0 002 2h12a2 2 0 002-2v-8M16 6l-4-4-4 4M12 2v13" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/></svg>
}
function BookmarkIcon({ s = 15 }: { s?: number }) {
  return <svg width={s} height={s} viewBox="0 0 24 24" fill="none"><path d="M6 3.5h12a1 1 0 011 1V21l-7-4-7 4V4.5a1 1 0 011-1z" stroke="currentColor" strokeWidth="1.8" strokeLinejoin="round"/></svg>
}
function PlayIcon({ s = 32 }: { s?: number }) {
...
function TicketIcon({ s = 15 }: { s?: number }) {
...
function MusicIcon({ s = 13 }: { s?: number }) {
  return <svg width={s} height={s} viewBox="0 0 24 24" fill="none"><path d="M9 18V5l12-2v13" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/><circle cx="6" cy="18" r="3" stroke="currentColor" strokeWidth="1.8"/><circle cx="18" cy="16" r="3" stroke="currentColor" strokeWidth="1.8"/></svg>
}
function SparkleIcon({ s = 12 }: { s?: number }) {
...
/* ════════════════════════════════════════════════════════════════════
   PLATFORM CONTENT MOCK-UP COMPONENTS
   ════════════════════════════════════════════════════════════════════ */
  ...
  return (
    <div className="relative mx-auto overflow-hidden rounded-2xl" style={{ width: 280, height: 496, background: `linear-gradient(160deg, ${piece.thumbnailColor} 0%, ${piece.gradientEnd} 100%)` }}>
      ...
      <button onClick={() => setPlaying(p => !p)}
        ...
        {!playing && (
          <div className="flex h-16 w-16 items-center justify-center rounded-full bg-black/30 backdrop-blur-sm text-white">
            <PlayIcon s={28}/>
          </div>
        )}
        {playing && (
          <div className="flex h-12 w-12 items-center justify-center rounded-full bg-black/20 text-white">
            <div className="flex gap-1.5"><div className="h-5 w-1.5 rounded-full bg-white"/><div className="h-5 w-1.5 rounded-full bg-white"/></div>
          </div>
        )}
      </button>
      ...
      <div className="absolute bottom-24 right-3 flex flex-col items-center gap-4">
        <div className="flex flex-col items-center gap-0.5">
          <div className="flex h-10 w-10 items-center justify-center rounded-full bg-black/20 text-white"><HeartIcon s={19}/></div>
          <span className="text-[10px] font-bold text-white">{piece.stats.likes}</span>
        </div>
        <div className="flex flex-col items-center gap-0.5">
          <div className="flex h-10 w-10 items-center justify-center rounded-full bg-black/20 text-white"><ChatIcon s={18}/></div>
          <span className="text-[10px] font-bold text-white">{piece.stats.comments}</span>
        </div>
        <div className="flex flex-col items-center gap-0.5">
          <div className="flex h-10 w-10 items-center justify-center rounded-full bg-black/20 text-white"><BookmarkIcon s={18}/></div>
          <span className="text-[10px] font-bold text-white">{piece.stats.saves}</span>
        </div>
        <div className="flex flex-col items-center gap-0.5">
          <div className="flex h-10 w-10 items-center justify-center rounded-full bg-black/20 text-white"><ShareIcon s={18}/></div>
          <span className="text-[10px] font-bold text-white">{piece.stats.shares}</span>
        </div>
        {/* Spinning music disc */}
        <div className="flex h-10 w-10 items-center justify-center rounded-full border-2 border-white/40 bg-black/30" style={{ animation: 'spin 4s linear infinite' }}>
          <MusicIcon s={14}/>
        </div>
      </div>
      ...
      <div className="absolute bottom-0 left-0 right-12 px-4 pb-8">
        ...
        <div className="mt-2 flex items-center gap-1.5">
          <MusicIcon s={11}/>
          <p className="text-[10.5px] text-white/70 truncate">original sound · {piece.creator.name}</p>
        </div>
      </div>
    ...
    <div className="mx-auto overflow-hidden rounded-2xl bg-black" style={{ width: 280 }}>
      ...
      <div className="flex items-center gap-2.5 bg-black px-3 py-2.5">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-[12px] font-bold text-white truncate">{piece.creator.handle}</p>
        </div>
      ...
      <div className="relative" style={{ height: 420, background: `linear-gradient(160deg, ${piece.thumbnailColor} 0%, ${piece.gradientEnd} 100%)` }}>
        <div className="absolute inset-0 opacity-[0.05]"
          style={{ backgroundImage: 'repeating-linear-gradient(0deg, transparent, transparent 28px, rgba(255,255,255,0.3) 28px, rgba(255,255,255,0.3) 29px)', backgroundSize: '100% 29px' }}/>
        ...
        <div className="absolute bottom-3 left-3 flex items-center gap-1">
          <EyeIcon s={12}/>
          <span className="text-[11.5px] font-bold text-white">{piece.stats.views} views</span>
        </div>
      </div>
      {/* IG action row */}
      <div className="flex items-center justify-between bg-black px-4 py-2.5">
        ...
        <button onClick={() => setSaved(s => !s)} className={saved ? 'text-white' : 'text-white'}><BookmarkIcon s={21}/></button>
      </div>
      {/* Likes + caption */}
      <div className="bg-black px-4 pb-4">
        ...
        <p className="mt-1 text-[12px] text-white/80 line-clamp-2">
          <span className="font-bold">{piece.creator.handle}</span> {piece.caption.split('\n')[0]}
        </p>
    ...
    <div className="mx-auto overflow-hidden rounded-xl bg-[#0f0f0f]" style={{ width: 320 }}>
      ...
      <div className="relative" style={{ height: 180, background: `linear-gradient(160deg, ${piece.thumbnailColor} 0%, ${piece.gradientEnd} 100%)` }}>
        ...
        <div className="absolute bottom-2 right-2 rounded bg-black/80 px-1.5 py-0.5">
          <span className="text-[10px] font-bold text-white">8:42</span>
        </div>
      </div>
      {/* Progress bar */}
      ...
      <div className="flex gap-3 p-3">
        <div className="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-full" style={{ background: piece.thumbnailColor }}>
          <span className="text-[11px] font-black text-white">{piece.creator.initials}</span>
        </div>
        <div className="flex-1 min-w-0">
...
export default function ContentReviewPage() {
  ...
  /* In real app: fetch by params.pieceId. Here using mock. */
  ...
  const handleApprove = () => {
    setStatus('approved')
    const approval: FeedbackEntry = {
      id: `fb${Date.now()}`, date: 'Just now', type: 'approval',
      text: feedback.trim() || 'Content approved — great work!',
    }
  ...
  const handleRequestRevision = () => {
    ...
    const entry: FeedbackEntry = {
      id: `fb${Date.now()}`, date: 'Just now', type: 'revision_request',
      text: feedback.trim(),
    }
  ...
  const handleSendNote = () => {
    ...
    const entry: FeedbackEntry = {
      id: `fb${Date.now()}`, date: 'Just now', type: 'note',
      text: feedback.trim(),
    }
    setPiece(prev => ({ ...prev, feedbackHistory: [...prev.feedbackHistory, entry] }))
    setFeedback('')
  ...
  const NAV_LEFT  = [
    ...
    { label: 'Campaign',   active: false, action: () => router.push('/brand/campaign/cp1') },
    { label: 'Review',     active: true,  action: () => {} },
  ]
  const NAV_RIGHT = [
              ...
              {NAV_LEFT.map((n, i) => (
                <div key={n.label} className="flex items-center gap-0.5">
                  ...
                  <button onClick={n.action}
                    className={`rounded-lg px-2 py-1.5 text-[13px] font-semibold transition hover:bg-primary/[0.08] hover:text-primary sm:px-3 ${n.active ? 'text-primary' : 'text-ink/55'}`}>
                    {n.label}
                  </button>
                </div>
              ))}
      ...
      <main className="mx-auto max-w-[1080px] px-6 py-8">
        ...
        <div className="mb-6 flex items-start justify-between gap-4">
          <div className="flex items-start gap-3">
            <button onClick={() => router.push('/brand/campaign/cp1')}
          ...
          <div className="flex flex-shrink-0 items-center gap-1.5">
            <button
              disabled={!piece.creator.prevPieceId}
              onClick={() => piece.creator.prevPieceId && router.push(`/brand/campaign/cp1/review/${piece.creator.prevPieceId}`)}
            ...
            <button
              disabled={!piece.creator.nextPieceId}
              onClick={() => piece.creator.nextPieceId && router.push(`/brand/campaign/cp1/review/${piece.creator.nextPieceId}`)}
              className="flex h-9 w-9 items-center justify-center rounded-xl border border-primary/15 text-ink/50 transition hover:border-primary/30 hover:text-primary disabled:cursor-not-allowed disabled:opacity-30">
              <ChevRight s={14}/>
            </button>
          </div>
            ...
            <div className={`rounded-2xl border border-primary/10 bg-white p-5 ${CARD}`}>
              ...
              <div className="mt-3 flex flex-wrap gap-1.5">
                {piece.hashtags.map(h => (
                  <span key={h} className="rounded-lg bg-primary/[0.07] px-2.5 py-1 text-[12px] font-semibold text-primary">{h}</span>
                ))}
              </div>
            </div>
          </div>
          ...
          <div className="flex w-full flex-col gap-4 lg:w-[340px] lg:flex-shrink-0">
            ...
            <div className={`rounded-2xl border border-primary/10 bg-white p-5 ${CARD}`}>
              <div className="flex items-center gap-3 mb-4">
                <div className="flex h-11 w-11 flex-shrink-0 items-center justify-center rounded-full font-extrabold text-white text-[16px]"
                  style={{ background: piece.creator.color }}>{piece.creator.initials}</div>
                <div>
                ...
                {allDos.map(item => (
                  <button key={item.id} type="button" onClick={() => toggleCheck(item.id)}
                    ...
                    <span className={`flex-1 font-medium leading-snug ${checked[item.id] ? 'text-emerald-700' : 'text-ink/65'}`}>
                      {item.label}
                      {item.required && <span className="ml-1 text-rose-400 text-[10px]">*</span>}
                    </span>
                  </button>
                ))}
            ...
            <div className={`rounded-2xl border border-primary/10 bg-white p-5 ${CARD}`}>
              ...
              <p className="mb-3 text-[12px] text-ink/45 leading-[1.6]">This message will be sent directly to <span className="font-bold text-ink">{piece.creator.name}</span> via the Messages tab. Be specific and constructive.</p>
              <textarea
                ref={textareaRef}
                value={feedback}
              ...
              <div className="mt-3 flex flex-col gap-2.5">
                ...
                <button onClick={handleApprove} disabled={!requiredPassed}
                  className={`flex items-center justify-center gap-2 rounded-xl py-3.5 text-[13.5px] font-bold text-white transition ${
                    requiredPassed
                      ? `${GRAD_BTN} shadow-[0_8px_24px_-6px_rgba(139,49,232,0.45)] hover:-translate-y-0.5`
                      : 'cursor-not-allowed bg-ink/10 text-ink/30'
                  }`}>
                  <Check s={14}/>
                  {status === 'approved' ? 'Approved ✓' : 'Approve content'}
                </button>
                ...
                <button onClick={handleSendNote} disabled={!feedback.trim()}
                  className={`flex items-center justify-center gap-2 rounded-xl border py-2.5 text-[12.5px] font-bold transition ${
                    feedback.trim() ? 'border-primary/20 text-primary hover:bg-primary/[0.05]' : 'border-primary/8 text-ink/28 cursor-not-allowed'
                  }`}>
                  <SendIcon s={13}/>Send as note only
                </button>

                {/* Escalate */}
                <button onClick={() => router.push('/dispute/new')}
                    ...
                    return (
                      <div key={entry.id} className={`px-5 py-4 ${typeCfg.bg}`}>
                        <div className="flex items-center justify-between gap-2 mb-2">
                          <span className={`flex items-center gap-1.5 text-[11px] font-bold ${typeCfg.text}`}>
                            <span className={`h-1.5 w-1.5 rounded-full ${typeCfg.dot}`}/>{typeCfg.label}
                          </span>
