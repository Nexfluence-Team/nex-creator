...
/* ════════════════════════════════════════════════════════════════════
   Agency-managed Brand Dashboard
   app/agency/brand/[brandSlug]/dashboard/page.tsx
   Nexfluence v4, LIGHT
   ════════════════════════════════════════════════════════════════════

   This is the brand dashboard (/dashboard/brand) rendered through the
   agency's lens — the agency is operating this brand's account on
   their behalf under a full_management contract.

   TWO NEW PIECES REQUESTED BY THE USER, NOTHING ELSE CHANGES:

   1. BRAND SWITCHER DROPDOWN (in the header, left side)
      Replaces a plain "Dashboard" label with a button showing the
      CURRENT brand's name + logo + a chevron. Clicking opens a list
      of every brand this agency manages (Kinetics, Lumora Skincare,
      Forma Fit) so the agency can jump between brand dashboards
      without going back through their own dashboard first.

   2. BRAND MANAGEMENT STRIP (persistent banner, just below the header)
      A disclaimer banner stating plainly: "You are viewing Kinetics'
      dashboard — managed by Baltic Creators Agency on Kinetics' behalf."
      Includes the contract type badge and a link back to the agency's
      own dashboard, so there is never ambiguity about whose account
      this is or who is acting on it.

   EVERYTHING ELSE — CampaignStrip, ActiveCampaignsRow, StatCard,
   CreatorTargetCard, ViewsChart, NotificationsPanel, AllCampaignsModal —
   is a verbatim port of /dashboard/brand/page.tsx. Only the action
   handlers are rewired to agency routes (/agency/campaign/new,
   /agency/messages, /agency/campaign/[id]).
   ════════════════════════════════════════════════════════════════════ */
...
/* ─── Managed brands — for the switcher dropdown ─────────────────── */
interface ManagedBrandOption {
...
const MANAGED_BRANDS: ManagedBrandOption[] = [
  { id: 'mb1', slug: 'kinetics',         name: 'Kinetics',        industry: 'Sports nutrition', color: '#8B31E8', initials: 'KI', contractType: 'full_management' },
  { id: 'mb2', slug: 'lumora-skincare',  name: 'Lumora Skincare', industry: 'Beauty',           color: '#059669', initials: 'LS', contractType: 'full_management' },
  { id: 'mb3', slug: 'forma-fit',        name: 'Forma Fit',       industry: 'Fitness apparel',  color: '#2563EB', initials: 'FF', contractType: 'single_campaign' },
]
/* Active context for this page render — Kinetics, agency's flagship full-management client */
const ACTIVE_BRAND = MANAGED_BRANDS[0]!
...
/* ─── Views data — identical shape to brand page ─────────────────── */
...
const INITIAL_NOTIFICATIONS: NotificationItem[] = [
  ...
  { id: 'n3', type: 'payout',       title: 'Payout sent — €350 to Amelia Roze',               time: '1d ago', unread: false },
  ...
  { id: 'n6', type: 'deal',         title: 'Rūta Vaitkutė requested to extend her partnership', time: '4d ago', unread: false },
]
...
const ACTIVE_CAMPAIGNS: ActiveCampaign[] = [
  {
    id: 'ac1', title: 'Electrolyte Hot Yoga',
    objective: 'Conversions', status: 'active',
    creators: 3, budget: '€2,600', endDate: 'Jul 15',
    metrics: { views: '735K', engagement: '9.1%', conversions: '4.1K' },
    color: '#8B31E8',
  },
  {
    id: 'ac2', title: 'Race Day Recovery',
    objective: 'Awareness', status: 'active',
    creators: 4, budget: '€1,800', endDate: 'Jul 20',
    metrics: { views: '620K', engagement: '10.4%', conversions: '1.9K' },
    color: '#2563EB',
  },
  {
    id: 'ac3', title: 'Strength Lab Q3',
    objective: 'Consideration', status: 'review',
    creators: 2, budget: '€950', endDate: 'Aug 1',
    metrics: { views: '210K', engagement: '6.2%', conversions: '680' },
    color: '#DB2777',
  },
]

/* ─── Perf constants ─────────────────────────────────────────────── */
const CURRENT_ACTIVE_CREATORS     = 7
const CURRENT_AVG_CONVERSION_RATE = 4.6
const CURRENT_AVG_ENGAGEMENT_RATE = 8.9
const CURRENT_PIECES_OF_CONTENT   = 19
...
function EyeIcon({ s = 18 }: { s?: number }) { return <svg width={s} height={s} viewBox="0 0 24 24" fill="none"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/><circle cx="12" cy="12" r="3" stroke="currentColor" strokeWidth="1.8"/></svg> }
function HandshakeIcon({ s = 18 }: { s?: number }) { return <svg width={s} height={s} viewBox="0 0 24 24" fill="none"><path d="M11 17l2 2a1 1 0 103-3" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/><path d="M14 14l2.5 2.5a1 1 0 103-3l-3.88-3.88a3 3 0 00-4.24 0l-.88.88a1 1 0 11-3-3l2.81-2.81a5.79 5.79 0 017.06-.87l.47.28a2 2 0 001.42.25L21 4" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/><path d="M21 3l1 11h-2M3 3l-1 11 6.5 6.5a1 1 0 103-3M3 4h8" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/></svg> }
...
function ImageIcon({ s = 18 }: { s?: number }) { return <svg width={s} height={s} viewBox="0 0 24 24" fill="none"><rect x="3" y="3" width="18" height="18" rx="2.5" stroke="currentColor" strokeWidth="1.8"/><circle cx="8.5" cy="8.5" r="1.5" stroke="currentColor" strokeWidth="1.6"/><path d="M21 15l-5-5L5 21" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/></svg> }
...
function SparkleIcon({ s = 16 }: { s?: number }) { return <svg width={s} height={s} viewBox="0 0 24 24" fill="none"><path d="M12 3v3M12 18v3M3 12h3M18 12h3M5.6 5.6l2.1 2.1M16.3 16.3l2.1 2.1M16.3 7.7l2.1-2.1M5.6 18.4l2.1-2.1" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/><circle cx="12" cy="12" r="3" stroke="currentColor" strokeWidth="1.8"/></svg> }
function PlayIcon({ s = 14 }: { s?: number }) { return <svg width={s} height={s} viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7z"/></svg> }
function ClockIcon({ s = 13 }: { s?: number }) { return <svg width={s} height={s} viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="9" stroke="currentColor" strokeWidth="1.8"/><path d="M12 7v5l3 3" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/></svg> }
function CheckIcon({ s = 11 }: { s?: number }) { return <svg width={s} height={s} viewBox="0 0 24 24" fill="none"><path d="M5 12l4 4 10-10" stroke="currentColor" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round"/></svg> }
function ChevronDownIcon({ s = 11 }: { s?: number }) { return <svg width={s} height={s} viewBox="0 0 24 24" fill="none"><path d="M6 9l6 6 6-6" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"/></svg> }
...
function RepeatIcon({ s = 10 }: { s?: number }) { return <svg width={s} height={s} viewBox="0 0 24 24" fill="none"><path d="M17 1l4 4-4 4M3 11V9a4 4 0 014-4h14M7 23l-4-4 4-4M21 13v2a4 4 0 01-4 4H3" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/></svg> }
function ZapSmallIcon({ s = 10 }: { s?: number }) { return <svg width={s} height={s} viewBox="0 0 24 24" fill="none"><path d="M4 14a1 1 0 01-.78-1.63l9.9-10.2a.5.5 0 01.86.46l-1.92 6.02A1 1 0 0013 10h7a1 1 0 01.78 1.63l-9.9 10.2a.5.5 0 01-.86-.46l1.92-6.02A1 1 0 0011 14z" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/></svg> }
...
/* ════════════════════════════════════════════════════════════════════
   BRAND SWITCHER DROPDOWN — the agency-only header addition
   Shows current brand (logo + name) with a chevron. Clicking opens a
   list of every managed brand; selecting one navigates to that
   brand's agency-managed dashboard.
   ════════════════════════════════════════════════════════════════════ */
function BrandSwitcherDropdown({ current, onSwitch }: {
  current: ManagedBrandOption
  ...
  return (
    <div ref={ref} className="relative">
      <button onClick={() => setOpen(o => !o)}
        className={`flex items-center gap-2 rounded-xl px-2 py-1.5 text-[13px] font-semibold transition hover:bg-primary/[0.08] sm:px-2.5 ${open ? 'bg-primary/[0.08] text-primary' : 'text-ink/70'}`}>
        <EntityTile initials={current.initials} color={current.color} size={22}/>
        <span className="hidden sm:inline">{current.name}</span>
        <ChevronDownIcon s={10}/>
      </button>

      {open && (
        <div className={`absolute left-0 top-[calc(100%+8px)] z-30 w-[260px] overflow-hidden rounded-2xl border border-primary/10 bg-white ${CARD}`}>
          <div className="border-b border-primary/8 px-4 py-3">
            <p className="text-[10.5px] font-black uppercase tracking-[0.14em] text-ink/35">Your managed brands</p>
          </div>
          <div className="max-h-[280px] overflow-y-auto">
            {MANAGED_BRANDS.map(b => {
              const active = b.slug === current.slug
              return (
                <button key={b.id} onClick={() => { onSwitch(b.slug); setOpen(false) }}
                  className={`flex w-full items-center gap-3 px-4 py-3 text-left transition hover:bg-primary/[0.04] ${active ? 'bg-primary/[0.05]' : ''}`}>
                  <EntityTile initials={b.initials} color={b.color} size={32}/>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5">
                      <p className={`truncate text-[13px] font-bold ${active ? 'text-primary' : 'text-ink'}`}>{b.name}</p>
                      {active && <span className={`flex h-4 w-4 flex-shrink-0 items-center justify-center rounded-full ${GRAD_BTN} text-white`}><CheckIcon s={9}/></span>}
                    </div>
                    <div className="flex items-center gap-1.5 mt-0.5">
                      <p className="text-[11px] text-ink/40">{b.industry}</p>
                      <span className={`flex items-center gap-0.5 rounded-full px-1.5 py-0.5 text-[9px] font-bold ${b.contractType === 'full_management' ? 'bg-emerald-50 text-emerald-700' : 'bg-sky-50 text-sky-700'}`}>
                        {b.contractType === 'full_management' ? <><RepeatIcon s={8}/>Managed</> : <><ZapSmallIcon s={8}/>Campaign</>}
                      </span>
                    </div>
                  </div>
                </button>
          ...
          <div className="border-t border-primary/8 px-4 py-3">
            <a href="/dashboard/agency" className="flex items-center gap-1.5 text-[12px] font-bold text-primary hover:underline">
              <BriefcaseIcon s={12}/>Back to agency dashboard
            </a>
          </div>
        </div>
      )}
...
/* ════════════════════════════════════════════════════════════════════
   BRAND MANAGEMENT STRIP — the disclaimer banner below the header
   States clearly: who this brand is, who manages it, and under what
   contract type. Sits above all dashboard content, full-width within
   the page container, on every load — not dismissible, since the
   agency-vs-brand context must never be ambiguous.
   ════════════════════════════════════════════════════════════════════ */
function BrandManagementStrip({ brand, agency }: { brand: ManagedBrandOption; agency: typeof AGENCY }) {
  return (
    <div className={`mb-6 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-primary/15 bg-primary/[0.04] px-5 py-3.5 ${CARD}`}>
      <div className="flex items-center gap-3">
        <span className={`flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-xl ${GRAD_BTN} text-white`}>
          <BriefcaseIcon s={16}/>
        </span>
        <p className="text-[13px] leading-[1.5] text-ink/70">
          You are viewing <span className="font-extrabold text-ink">{brand.name}'s</span> dashboard — managed by{' '}
          <span className="font-extrabold text-ink">{agency.name}</span> on {brand.name}'s behalf.
        </p>
      </div>
      <div className="flex flex-shrink-0 items-center gap-2">
        <span className={`flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[11px] font-bold ${brand.contractType === 'full_management' ? 'bg-emerald-50 text-emerald-700' : 'bg-sky-50 text-sky-700'}`}>
          {brand.contractType === 'full_management' ? <><RepeatIcon s={10}/>Full management</> : <><ZapSmallIcon s={10}/>Single campaign</>}
        </span>
        <a href="/dashboard/agency"
          className="rounded-full border border-primary/15 bg-white px-3 py-1 text-[11.5px] font-bold text-primary transition hover:bg-primary/[0.05]">
          Agency dashboard
        </a>
      </div>
    </div>
...
/* ═══════════════════════ CAMPAIGN STRIP — verbatim, agency CTA ═════ */
  ...
  return (
    <button type="button" onClick={onClick}
      ...
      style={{
        ...
        boxShadow: hovered ? '0 16px 40px -12px rgba(139,49,232,0.28), 0 2px 8px -2px rgba(10,6,18,0.06)' : undefined,
      }}>
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-2.5">
          <div className="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-xl" style={{ background: `${campaign.color}18` }}><PlayIcon s={14}/></div>
          <span className="line-clamp-2 text-[13.5px] font-extrabold leading-tight text-ink">{campaign.title}</span>
        </div>
      ...
      <div className="mt-3 flex items-center gap-2">
        ...
        <span className="flex items-center gap-1 text-[11px] font-medium text-ink/40"><ClockIcon s={11}/>Ends {campaign.endDate}</span>
      </div>
      ...
      <div className="mt-4 flex items-center justify-between border-t border-primary/8 pt-3.5">
        <span className="flex items-center gap-1.5 text-[11.5px] font-medium text-ink/50"><UsersIcon s={12}/>{campaign.creators} creator{campaign.creators !== 1 ? 's' : ''}</span>
...
function ActiveCampaignsRow({ onCampaignClick, onViewAll }: { onCampaignClick: (id: string) => void; onViewAll: () => void }) {
  return (
    <div className={`rounded-2xl border border-primary/10 bg-white p-5 ${CARD}`}>
      <div className="mb-4 flex items-center justify-between">
        ...
        <button onClick={onViewAll} className="hidden items-center gap-1.5 rounded-lg border border-primary/15 px-3.5 py-2 text-[12px] font-bold text-primary transition hover:bg-primary/[0.04] sm:flex">
          View all<ArrowRightIcon s={12}/>
        </button>
      </div>
      <div className="flex gap-3 overflow-x-auto pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        {ACTIVE_CAMPAIGNS.map(c => <CampaignCard key={c.id} campaign={c} onClick={() => onCampaignClick(c.id)}/>)}
...
/* ═══════════════════════ CREATOR TARGET CARD — verbatim ════════════ */
function GoalSelect<T extends number>({ label, value, options, onChange }: {
        ...
        <div className={`absolute left-0 top-[calc(100%+6px)] z-20 w-[110px] overflow-hidden rounded-xl border border-primary/10 bg-white ${CARD}`}>
          {options.map(opt => (
            <button key={opt.value} onClick={() => { onChange(opt.value); setOpen(false) }}
              ...
              {opt.label}{value === opt.value && <span className={`h-2 w-2 rounded-full ${GRAD_BTN}`}/>}
            </button>
          ))}
        </div>
      )}
        ...
        {[
          ...
          <div key={bar.label} className="rounded-xl bg-surface-sub p-4">
            <div className="flex items-end justify-between mb-2">
              ...
              <span className={`text-[12px] font-bold ${bar.cur >= bar.tgt ? 'text-emerald-600' : 'text-ink/55'}`}>{bar.cur}% <span className="font-medium text-ink/35">/ {bar.tgt}%</span></span>
            </div>
            <div className="h-2.5 w-full overflow-hidden rounded-full bg-primary/[0.08]">
              <div className={`h-full rounded-full transition-all duration-700 ${bar.cur >= bar.tgt ? 'bg-gradient-to-r from-emerald-400 to-emerald-500' : GRAD_BTN}`} style={{ width: `${bar.prog}%` }}/>
            </div>
            <p className="mt-1.5 text-[11px] text-ink/35">{bar.cur >= bar.tgt ? '✓ Target met' : `${(bar.tgt - bar.cur).toFixed(1)}% gap remaining`}</p>
          </div>
        ))}
      </div>
        ...
        {allMet ? (
          ...
          <>
            <div className={`flex items-center gap-2 rounded-xl px-4 py-2.5 ${GRAD_BTN}`}>
              <UsersIcon s={15}/><span className="text-[13px] font-bold text-white">{needed} more creator{needed !== 1 ? 's' : ''} needed</span>
            </div>
            <div className="flex items-center gap-2 rounded-xl bg-surface-sub px-4 py-2.5">
              <ImageIcon s={15}/><span className="text-[13px] font-semibold text-ink/70"><span className="font-extrabold text-ink">{needed * contentPerCreator}</span> more piece{needed * contentPerCreator !== 1 ? 's' : ''} of content</span>
            </div>
            <div className="flex items-center gap-2 rounded-xl bg-surface-sub px-4 py-2.5">
              <UsersIcon s={15}/><span className="text-[13px] font-semibold text-ink/70"><span className="font-extrabold text-ink">{CURRENT_ACTIVE_CREATORS + needed}</span> total creators to hit targets</span>
            </div>
            <p className="ml-1 text-[11.5px] text-ink/35">At {contentPerCreator} piece{contentPerCreator !== 1 ? 's' : ''}/creator · targets: {targetConversion}% CVR, {targetEngagement}% ER</p>
          </>
        )}
      </div>
    </div>
    ...
    <div className={`flex h-full w-full flex-col rounded-2xl border border-primary/10 bg-white p-6 ${CARD}`}>
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-[12px] font-medium text-ink/40">{delta ? `vs previous ${range} days` : `Last ${range} days`} · creators discovering this profile</p>
        </div>
...
/* ═══════════════════════ ALL CAMPAIGNS MODAL — verbatim ════════════ */
function AllCampaignsModal({ open, onClose, onSelect }: { open: boolean; onClose: () => void; onSelect: (id: string) => void }) {
  useEffect(() => {
    if (!open) return
  ...
  return (
    <div className="fixed inset-0 z-[600] flex items-center justify-center p-4 sm:p-6"
      ...
      <div className={`relative z-10 flex w-full max-w-[760px] flex-col overflow-hidden rounded-3xl bg-white ${CARD}`} style={{ maxHeight: 'min(90vh, 720px)' }}>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5">
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            {ACTIVE_CAMPAIGNS.map(c => {
              ...
              return (
                <button key={c.id} type="button" onClick={() => { onClose(); onSelect(c.id) }}
                  ...
                  <div className="flex items-start gap-3">
                    <div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-xl" style={{ background: `${c.color}18` }}><PlayIcon s={15}/></div>
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="mt-1.5 flex flex-wrap items-center gap-2">
                        ...
                        <span className={`flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-[10.5px] font-bold ${st.bg} ${st.text}`}><span className={`h-1.5 w-1.5 rounded-full ${st.dot}`}/>{st.label}</span>
                      </div>
                    </div>
                  </div>
                  <div className="mt-4 grid grid-cols-3 gap-2 border-t border-primary/8 pt-4">
                  ...
                  <div className="mt-4 flex items-center justify-between border-t border-primary/8 pt-3.5">
                    <span className="flex items-center gap-1.5 text-[11.5px] font-medium text-ink/50"><UsersIcon s={12}/>{c.creators} creator{c.creators !== 1 ? 's' : ''}</span>
                    <div className="flex items-center gap-2.5">
...
/* ═══════════════════════════════════════════════════════════════════
   PAGE
   ═══════════════════════════════════════════════════════════════════ */
export default function AgencyBrandDashboardPage() {
  const router = useRouter()
  const [range,            setRange]            = useState<RangeOption>(7)
  const [notifications,    setNotifications]    = useState<NotificationItem[]>(INITIAL_NOTIFICATIONS)
  const [allCampaignsOpen, setAllCampaignsOpen] = useState(false)

  /* In real app: derive from params.brandSlug. Mock uses ACTIVE_BRAND (Kinetics). */
  const currentBrand = ACTIVE_BRAND
  ...
  const goToMessages    = () => router.push('/agency/messages')
  const goToNewCampaign = () => router.push('/agency/campaign/new')
  const goToCampaign    = (id: string) => id === 'new' ? goToNewCampaign() : router.push(`/agency/campaign/${id}`)
  const switchBrand     = (slug: string) => router.push(`/agency/brand/${slug}/dashboard`)

  return (
    <div className="min-h-screen bg-canvas font-rubik text-ink antialiased">

      <AllCampaignsModal open={allCampaignsOpen} onClose={() => setAllCampaignsOpen(false)} onSelect={goToCampaign}/>

      {/* ════ HEADER — brand switcher replaces plain "Dashboard" label ════ */}
      <header className="sticky top-0 z-40 border-b border-primary/10 bg-white/95 backdrop-blur-xl">
        <div className="mx-auto max-w-[1080px] px-4 pb-3 pt-3 sm:px-6">
          <div className="relative flex w-full items-center justify-between rounded-2xl px-3 py-2.5" style={{ overflow: 'visible' }}>
            ...
            {/* Left nav — brand switcher + Discover Creators */}
            <div className="relative z-10 flex items-center gap-0.5">
              <BrandSwitcherDropdown current={currentBrand} onSwitch={switchBrand}/>
              <button className="flex items-center gap-1.5 rounded-lg px-2 py-1.5 text-[13px] font-semibold text-ink/70 transition hover:bg-primary/[0.08] hover:text-primary sm:px-3.5">
                Discover Creators
              </button>
            </div>
            ...
            <div className="relative z-10 flex items-center gap-1.5">
              ...
              <button title="Notifications" aria-label="Notifications"
                className="relative flex h-9 w-9 items-center justify-center rounded-lg text-ink/60 transition hover:bg-primary/[0.08] hover:text-primary">
                <BellIcon s={18}/>
                {unreadNotifs > 0 && <span className={`absolute -right-0.5 -top-0.5 flex h-[15px] min-w-[15px] items-center justify-center rounded-full px-1 text-[8.5px] font-black text-white ${GRAD_BTN}`}>{unreadNotifs}</span>}
              </button>
      ...
      <main className="mx-auto max-w-[1080px] px-6 py-8">
        ...
        {/* ════ BRAND MANAGEMENT STRIP — the disclaimer banner ════ */}
        <BrandManagementStrip brand={currentBrand} agency={AGENCY}/>

        {/* Title row */}
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <h1 className="text-[clamp(22px,3.2vw,30px)] font-black tracking-[-0.03em] text-ink">
              {currentBrand.name}'s performance <span aria-hidden="true">📊</span>
            </h1>
            <p className="mt-1 text-[14px] text-ink/55">Here's how {currentBrand.name}'s creator partnerships have been performing.</p>
          </div>
          <a href={`/brand/${currentBrand.slug}`}
        ...
        {/* Campaign strip */}
        <div className="mt-6"><CampaignStrip onClick={goToNewCampaign}/></div>

        {/* Running campaigns */}
        <div className="mt-4"><ActiveCampaignsRow onCampaignClick={goToCampaign} onViewAll={() => setAllCampaignsOpen(true)}/></div>

        {/* 4 stat cards */}
        <div className="mt-6 grid grid-cols-2 gap-4 sm:grid-cols-4">
          <StatCard icon={<UsersIcon s={18}/>}  label="Active creators"      value={String(CURRENT_ACTIVE_CREATORS)}   sublabel="Currently deployed"         delta={{ label: '+2 this month',        positive: true }}/>
          <StatCard icon={<ZapIcon s={18}/>}    label="Avg. conversion rate" value={`${CURRENT_AVG_CONVERSION_RATE}%`} sublabel="Across all active creators" delta={{ label: '+0.4% vs last month', positive: true }}/>
          <StatCard icon={<EyeIcon s={18}/>}    label="Avg. engagement rate" value={`${CURRENT_AVG_ENGAGEMENT_RATE}%`} sublabel="Across all active creators" delta={{ label: '+0.9% vs last month', positive: true }}/>
          <StatCard icon={<ImageIcon s={18}/>}  label="Pieces of content"    value={String(CURRENT_PIECES_OF_CONTENT)} sublabel="Published this month"       delta={{ label: '+4 this week',         positive: true }}/>
        </div>

        {/* Creator target card */}
        <div className="mt-4"><CreatorTargetCard/></div>

        {/* Views chart + Notifications */}
        <div className="mt-6 grid grid-cols-1 items-stretch gap-6 lg:grid-cols-3">
          <div className="flex h-full lg:col-span-2"><ViewsCard range={range} onRangeChange={setRange}/></div>
